...
global with Sharing class ProjectController {
    ...
    global static Tasky_Task__c[] getTasks(Id projectId) {
        Tasky_Task__c[] tasks = [SELECT Name, Assignee__c, Detail__c, Late__c, Status__c, Assignee__r.User__c, Due_Date__c, Completed_Date__c, Project__c FROM Tasky_Task__c WHERE Project__c =: projectId ORDER BY Due_Date__c ASC NULLS LAST];
        return tasks;
    }
